...
empty_dict = {'AMETHYSTS' : 0, 'STARFRUIT' : 0, 'ORCHIDS' : 0, 'CHOCOLATE':0, 'ROSES':0, 'STRAWBERRIES':0, 'GIFT_BASKET':0}
...
class Trader:
    ...
    def compute_orders_ORCHID(self, product, order_depth, acc_bid, acc_ask):
        ...
        if(self.ORCHID_ask-acc_bid<=self.ORCHID_gap-self.ORCHID_gapth and self.POSITION_LIMIT[product]>self.position[product]):
            orders.append(Order(product, acc_bid, -(self.POSITION_LIMIT[product]-abs(self.position[product]))))
        return orders
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        self.ORCHID_gap = -state.observations.conversionObservations["ORCHIDS"].importTariff-state.observations.conversionObservations["ORCHIDS"].transportFees
