...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        print(f"Start transmission")
        print(f"State_timestamp: {state.timestamp}")
        print(f"State_listings: {state.listings}")
        state_order_depths = {key: (value.buy_orders, value.sell_orders) for key, value in zip(state.order_depths.keys(), state.order_depths.values())}
        print(f"State_order_depths: {state_order_depths}")
        print(f"State_own_trades: {state.own_trades}")
        print(f"State_market_trades: {state.market_trades}")
        print(f"State_position: {state.position}")
        print(f"plainValueObservations: {state.observations.plainValueObservations}")
        conversionObservations = {key: (value.bidPrice, value.askPrice, value.transportFees, value.exportTariff, value.importTariff, value.sunlight, value.humidity) for key, value in zip(state.observations.conversionObservations.keys(), state.observations.conversionObservations.values())}
        print(f"conversionObservations: {conversionObservations}")
        ...
        print(f'bs_STARFRUIT: {bs_STARFRUIT}, bb_STARFRUIT: {bb_STARFRUIT}')
        ...
        if len(self.STARFRUIT_cache) == self.STARFRUIT_dim:
            ...
            print(f'Next price is {next_price}')
